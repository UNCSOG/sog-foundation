		...
		<div class="report_filter_wrapper">
			...
			<div class="report_filters py-2">
				<?php foreach ($filters as $filter) { ?>
					<?php
						$where_based_on_other_filters=implode(" ",$data['array_where_based_on_other_filters'] ?? array());
					...
					>
						...
						?>
						<i class="fas fa-times-circle text-secondary sog_ex_add_explore_filter remove_filter clickable"
			...
			<?php foreach ($data['fields'] as $field) { ?>
				<?php 
					if (in_array($field['COLUMN_NAME'],$filters_array)) {
						$icon_color="fas text-primary ";
					}else{
						$icon_color="far text-secondary";
					}
				?>
				<div class="form-check filter_row">
						<i class=" fa-filter <?php echo $icon_color;?> sog_ex_add_explore_filter clickable"
					...
					<input class="form-check-input sog_ex_report_checkbox" type="checkbox" value="<?php echo $data['table_name'].".`".$field['COLUMN_NAME'];?>`"
						title="Toggle field in the table"
					>
