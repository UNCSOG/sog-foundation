...
$(document).ready(function() {
	...
	$(this).on("click", '.report_checkbox',function(){
		...
		//if the delay option is selected, then don't automatically run the report.
		if (!$("#click_to_load").is(":checked")){
			//now run the report
	...
	if (table_name){
		...
		//update table name display only
		$(".selected_table_name").html(table_name);
	...
	$(this).on("click", '.save_remote_db_option',save_remote_db_option);
...
function select_report_table(elem){
	...
	//this is specific for dwi site
	var db=getParameterByName("db");
...
function process_report_request(){
	...
	$.ajax({ url: plugin_dir()+'/process.php',
		...
		beforeSend: function(){
			...
			//load initial spinner if table isn't already loaded
			if (!$(".refresh_table_icon").is(":visible")){
				$(".report_table_here").html('<div class="text-center text-success"><i class="fas fa-sync fa-spin "></i></div>');
			}
...
function save_remote_db_option(){
	if ($("#remote_db_option").is(":checked")) {
		var value=1;
	}else{
		var value=0;
	}
	var data_to_send = [];
	data_to_send.push(value); 
	data_to_send.push($("#remote_db_option_hostname").val()); 
	data_to_send.push($("#remote_db_option_port").val()); 
	data_to_send.push($("#remote_db_option_username").val()); 
	data_to_send.push($("#remote_db_option_password").val()); 
	data_to_send.push($("#remote_db_option_db_name").val()); 
	data_to_send.push($("#remote_db_option_label").val()); 
	if (getParameterByName("debug",null)){
		console.log(data_to_send);
	}
	$.ajax({ url: plugin_dir()+'/process.php',
		type: 'POST',
		data    : {save_remote_db_option:data_to_send},
		beforeSend: function(){
			$("#save_remote_db_message").html("Checking Connection");
		},
		success: function(output) {
			if (getParameterByName("debug",null)){
				console.log(output);
			 ...
			 if(output.status == "success"){
				save_flash($(".save_remote_db_option"));

				//reload page so tables data will update
				location.reload();
			 }else{
				if (output.check_valid_remote_db && output.check_valid_remote_db.error_message) {
					$("#save_remote_db_message").html(output.check_valid_remote_db.error_message);
				}
				$("#remote_db_option").prop('checked', false);
				if (output.alert_message) {
					alert(output.alert_message);
				}
			 }
	...
	$.ajax({ url: plugin_dir()+'/process.php',
		...
		success: function(output) {
			...
			if(output.status == "success"){
				...
				if (output.check_if_stale_data && output.check_if_stale_data.is_stale){
					var stale_options=[];
					stale_options['browser_alert']=true;
					process_is_stale(stale_options);
				}else{
					//if there is a value to restore
					if (output.restore_result && output.restore_result.value){
