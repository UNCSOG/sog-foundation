...
if(isset($_POST['generic_update']) && !empty($_POST['generic_update'])) {
    ...
    if (!$value) {
        $value = null;
    } //need this for something but can't remember what for
    ...
    if ($allow) { //if permission
        if ($si == 's' or $si == 'ss') {
            if ($si == 's') {
                $result = update_generic_table_s(['table_name' => $table_name, 'field_name' => $field_name, 'id' => $id, 'value' => $value, 'id_column_name' => $id_column_name]);
                $response_array['result'] = $result;
                $response_array['arr'] = ['table_name' => $table_name, 'field_name' => $field_name, 'id' => $id, 'value' => $value, 'id_column_name' => $id_column_name];
            } else {
                $result = update_generic_table_ss(['table_name' => $table_name, 'field_name' => $field_name, 'id' => $id, 'value' => $value, 'id_column_name' => $id_column_name]);
            }
            ...
            if ($si == 'is') {
                $result = update_generic_table_is(['table_name' => $table_name, 'field_name' => $field_name, 'id' => $id, 'value' => $value, 'id_column_name' => $id_column_name]);
            } else {
                $result = update_generic_table_i(['table_name' => $table_name, 'field_name' => $field_name, 'id' => $id, 'value' => $value, 'id_column_name' => $id_column_name]);
            }
            if ($result) {
                $use_the_log = 1;
                $response_array['status'] = 'success';
...
if(isset($_POST['sog_settings_load_menu_section']) && !empty($_POST['sog_settings_load_menu_section'])) {
    ...
    if (function_exists("sog_settings\display_" . $slug)) {
        call_user_func_array("sog_settings\display_" . $slug, [null]);
    } else {
...
if(isset($_POST['generic_table_add_new']) && !empty($_POST['generic_table_add_new'])) {
    ...
    //check if being used.
    $being_used = get_generic_sql(['table_name' => $table_name, 'fields' => $field_name, 'limit' => 'limit 1', 'where' => 'and ' . $field_name . '="' . $value . '"']);
    ...
    if (!$failed_allowed_tags) {
        if ($allow) {
            if (!$allow_duplicates and $being_used) {
                ...
                if ($si == 's' or $si == 'ss') {
                    if ($si == 's') {
                        $new_id = add_row_generic_table_s(['field_name' => $field_name, 'table_name' => $table_name, 'value' => $value, 'ignore_last_inserted_id' => $ignore_last_inserted_id]);
                        if ($new_id) {
                            $use_the_log = 1;
                        }
                    }
                ...
                if ($new_id and $reload_with_ajax) {
                    ...
                    if ($use_generic_manage) {
                        display_setting_manage(['table_display_name' => $table_display_name, 'table_name' => $table_name]);
                    } else {
                        ...
                        if (function_exists($get_fx) and function_exists($display_fx)) {
                            ...
                            $table_data = call_user_func_array($get_fx, [['order_by' => 'id desc']]);
                            call_user_func_array($display_fx, [['table_data' => $table_data]]);
                        }
                    }
                    $html = ob_get_clean();
                    $response_array['html'] = $html;
...
if(isset($_POST['sog_settings_delete']) && !empty($_POST['sog_settings_delete'])) {
    ...
    if ($allow) {
        if ($change_status_on_delete) {
            ...
            $delete_result = update_generic_table_i(['table_name' => $table_name, 'id' => $id, 'id_column_name' => $id_column_name, 'field_name' => 'status', 'value' => 0]);
        } else {
            $delete_result = delete_generic_table_row(['table_name' => $table_name, 'id' => $id, 'id_column_name' => $id_column_name]);
...
if(isset($_POST['sog_settings_update_setting']) && !empty($_POST['sog_settings_update_setting'])) {
    ...
    //make change
    $result = update_generic_table_s(['table_name' => 'sog_settings', 'field_name' => 'value', 'id' => $setting_id, 'value' => $value, 'id_column_name' => 'id']);
