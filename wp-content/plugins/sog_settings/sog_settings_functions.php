...
function settings_page(){
    ...
    $menu_items=array(
        array("name"=>"Instructions","slug"=>"setting_instructions"),
        array("name"=>"Settings","slug"=>"setting_settings"),
        array("name"=>"Manage Settings","slug"=>"setting_manage"),
       );
...
function display_setting_manage($data){
    ...
    $third_field="";
    $add_new_row=1;
    $input_col=array(1,3,4,5,6);
    $checkbox_col=array(2);
    $skinny_col=array(0);
    $skip_col=array();
    $col_names=array("ID","Name","Is Bool","Value","Category","Sort","Description");
    $table_data=get_generic_sql(array("table_name"=>$data['table_name'],"fields"=>"id, name, is_bool, value, category, sort, description","order_by"=>"order by category,sort,id desc"));
...
function create_sql_tables(){
	...
	$return['sql_report_tables']=generic_sql_query(array("sql"=>$sql_settings_table));
...
function get_generic_sql($data) {
	...
	$fx=__FUNCTION__;
	$con = db_connect(null) or die("Couldn't connect to db. ".$db_name."-".$fx);
				...
				$table_html.= "<tr>";
					...
					foreach ($headers as $header) {
						if (!in_array($th_count,$skip_col)) {
							$skinny_col_style="";
							if (in_array($th_count,$skinny_col)) {$skinny_col_style="skinny_col";}
						...
						$table_html.= "<tr class='filter_row ".$table_name."_row_".$rows[$id_column_name]."'>";
							...
							foreach ($rows as $row) {
								if (!in_array($col_count,$skip_col)) {
									$skinny_col_style="";
									if (in_array($col_count,$skinny_col)) {$skinny_col_style="skinny_col";}
...
function is_setting($setting_id) {
	...
	$sql_result = $stmt->get_result();
	$num_rows = mysqli_num_rows($sql_result);$cols=[];while ($row = $sql_result->fetch_array(MYSQLI_BOTH)) {
		if ($num_rows) {
			return true;
		}else {
			return false;
...
function get_setting($setting_id) {
	...
	$sql_result = $stmt->get_result();
	$num_rows = mysqli_num_rows($sql_result);
	$cols=[];while ($row = $sql_result->fetch_array(MYSQLI_BOTH)) {
		return $row['value'];
	}	
}