...
* The Utility Bar - the utility bar is a common header item that contains links to other common sites within the unc networks
...
An embedded web widget for sites that do not use WordPress, [Click here for more information about the standalone Utility Bar](src/webscript-utility-bar?ref_type=heads)
...
To watch while you work run `npm run start` builds using source maps, not production ready.  building is faster and automatic when you save a file in the watch list
...
A banner is shown on all pages where the plugin in active when the publish status of the cap message is set to "published".  When a "normal" cap alert is sent from rave, the alert is logged but the stored message is just set to "unpublished".  A banner publish status can be set using the WP CLI commands or the REST API commands detailed below.
...
```
* [string] target is a dom identifier of some location on the page the skip link should go to.  IDs work best, defaults to #maincontent
* [string] $actype is a string, possibilities are 'weather', 'adverse', 'emergency', 'crime'
* [\Alert_Service\Cap] $cap is a \Alert_Service\Cap object.
...
```
* [array] $data, an array of all the data going to the banner template
* [string] $actype is a string, possibilities are 'weather', 'adverse', 'emergency', 'crime'
* [\Alert_Service\Cap] $cap is a \Alert_Service\Cap object.
...
## School of Government Additions

The defaults if nothing selected go to what the original UNC Utility Bar plugin was set to.

1. Added the ability to align the menu items left, center or right.
2. Added the ability to add our own menu items.
3. Added the ability to enable or disable the menu items that came from UNC ITS original unc-utility-bar plugin.
4. Added the ability to enable or disable the menu title for both mobile and desktop.
   1. Added a short title for mobile if one is wanted.
5. Can control the utility bar width so it matches your site.


## SCSS Build/Compile Instructions

To compile the SCSS files for this plugin, follow these steps:
...
2. **Build the frontend assets (including SCSS):**
   ```sh
   npm run build:frontend
   ```
   This will compile the SCSS and JS for the plugin's frontend assets. The output will be placed in the `build/frontend/` directory.

### Troubleshooting
- If you get an error about a missing script, check your `package.json` for the correct build script name (e.g., `build:frontend`).
- If you want to add a custom SCSS build script, you can use [Sass](https://sass-lang.com/install):
   ```sh
