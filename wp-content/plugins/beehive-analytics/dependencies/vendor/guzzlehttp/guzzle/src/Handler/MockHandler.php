...
class MockHandler implements \Countable
{
    ...
    public function __invoke(RequestInterface $request, array $options)
    {
        ...
        $response = $response instanceof \Exception ? \Beehive\GuzzleHttp\Promise\rejection_for($response) : \Beehive\GuzzleHttp\Promise\promise_for($response);
        return $response->then(function ($value) use($request, $options) {
            ...
            if (isset($options['sink'])) {
                ...
                } elseif ($sink instanceof \Beehive\Psr\Http\Message\StreamInterface) {
                    $sink->write($contents);
                }
            }
            return $value;
            ...
            return \Beehive\GuzzleHttp\Promise\rejection_for($reason);
        });
    }
    /**
    ...
    public function append()
    {
        foreach (\func_get_args() as $value) {
            if ($value instanceof ResponseInterface || $value instanceof \Exception || $value instanceof PromiseInterface || \is_callable($value)) {
                ...
                throw new \InvalidArgumentException('Expected a response or ' . 'exception. Found ' . \Beehive\GuzzleHttp\describe_type($value));
            }
        }
    }
    /**
