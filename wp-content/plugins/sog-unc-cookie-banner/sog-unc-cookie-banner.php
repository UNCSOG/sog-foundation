<?php
/*
 * Plugin Name: SOG UNC Cookie Banner
 * Description: Minor update allows pages to be excluded. Display a cookie banner on all sites. Can be used on both multisite and single site installs.
 * Author: ITS Digital Services - Michael Triplett
 * Editor: Lindsay Hoyt | University of North Carolina at Chapel Hill - School of Government
 * Version: 0.0.1
 */
...
class UNCCookieBanner {
    ...
    public function settings_page() {
        ...
        if ($can_manage && isset($_POST['submit']) && isset($_POST['unc_cookie_banner_settings_nonce']) && wp_verify_nonce($_POST['unc_cookie_banner_settings_nonce'], 'unc_cookie_banner_settings_action')) {
            $banner_text = wp_kses_post(wp_unslash($_POST['unc_cookie_banner_text']));
            if (empty($banner_text)) {
                $banner_text = "This website uses cookies and similar technologies to understand visitor experiences. By using this website, you consent to UNC-Chapel Hill's cookie usage in accordance with their <a href='https://www.unc.edu/about/privacy-statement/' title=''>Privacy Notice</a>.";
            }
            $button_text = sanitize_text_field($_POST['unc_cookie_banner_button_text']);
            if (empty($button_text)) {
                $button_text = "I Accept";
            }
            $exclusion_list_blogs = isset($_POST['unc_cookie_banner_exclude_blogs']) ? sanitize_text_field($_POST['unc_cookie_banner_exclude_blogs']) : '';
            $exclusion_list_pages = isset($_POST['unc_cookie_banner_exclude_pages']) ? sanitize_text_field($_POST['unc_cookie_banner_exclude_pages']) : '';

            $this->update_option('unc_cookie_banner_text', $banner_text);
            $this->update_option('unc_cookie_banner_button_text', $button_text);
            $this->update_option('unc_cookie_banner_exclude_blogs', $exclusion_list_blogs);
            $this->update_option('unc_cookie_banner_exclude_pages', $exclusion_list_pages);
        }

        $unc_cookie_banner_text = $this->get_option('unc_cookie_banner_text', "This website uses cookies and similar technologies to understand visitor experiences. By using this website, you consent to UNC-Chapel Hill's cookie usage in accordance with their <a href='https://www.unc.edu/about/privacy-statement/' title=''>Privacy Notice</a>.");
        ...
        $unc_cookie_banner_exclude_pages = $this->get_option('unc_cookie_banner_exclude_pages', '');

        $nonce_field = wp_nonce_field('unc_cookie_banner_settings_action', 'unc_cookie_banner_settings_nonce', true, false);
    ...
    public function enqueue_scripts() {
        ...
        $exclusion_list_pages = array_map('trim', explode(',', $unc_cookie_banner_exclude_pages));
        // Checking if the current blog or page ID is in the exclusion list.
        if (in_array((string) $current_blog_id, $exclusion_list, true) || in_array((string) get_queried_object_id(), $exclusion_list_pages, true)) {
            return; // If the current blog or page is in the exclusion list, we don’t enqueue scripts and exit.
        }
