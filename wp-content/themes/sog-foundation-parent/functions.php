...
if ( ! function_exists( 'sog_master_setup_theme' ) ) {
	/**
	 ...
	 * @return void
	 */
	function sog_master_setup_theme() {
	...
	add_action( 'after_setup_theme', 'sog_master_setup_theme' );
...
if ( ! function_exists( 'sog_master_add_user_fields' ) ) {
	/**
	 ...
	 * @return array
	 */
	function sog_master_add_user_fields( $fields ) {
		// Add new fields.
	...
	add_filter( 'user_contactmethods', 'sog_master_add_user_fields' );
}
 ...
 * @return bool
 */
function sog_master_filter_media_comment_status( $open, $post_id = null ) {
...
add_filter( 'comments_open', 'sog_master_filter_media_comment_status', 10, 2 );
 ...
 * @return string
 */
function sog_master_custom_edit_post_link( $link ) {
	return str_replace( 'class="post-edit-link"', 'class="post-edit-link badge bg-secondary"', $link );
}
add_filter( 'edit_post_link', 'sog_master_custom_edit_post_link' );
 ...
 */
function sog_master_custom_edit_comment_link( $link ) {
	return str_replace( 'class="comment-edit-link"', 'class="comment-edit-link badge bg-secondary"', $link );
}
add_filter( 'edit_comment_link', 'sog_master_custom_edit_comment_link' );
 ...
 * @return string
 */
function sog_master_oembed_filter( $html ) {
	return '<div class="ratio ratio-16x9">' . $html . '</div>';
}
add_filter( 'embed_oembed_html', 'sog_master_oembed_filter', 10 );

if ( ! function_exists( 'sog_master_content_nav' ) ) {
	/**
	 ...
	 */
	function sog_master_content_nav( $nav_id ) {
		global $wp_query;
 ...
 * @return void
 */
function sog_master_widgets_init() {
	// Area 1.
	register_sidebar(
		array(
...
add_action( 'widgets_init', 'sog_master_widgets_init' );

if ( ! function_exists( 'sog_master_article_posted_on' ) ) {
	/**
	 * "Theme posted on" pattern.
	 *
	 * @since v1.0
	 */
	function sog_master_article_posted_on() {
		printf(
 ...
 * @return string
 */
function sog_master_password_form() {
	global $post;
...
add_filter( 'the_password_form', 'sog_master_password_form' );


if ( ! function_exists( 'sog_master_comment' ) ) {
	/**
	 * Style Reply link.
	 *
	 * @since v1.0
	 ...
	 * @return string
	 */
	function sog_master_replace_reply_link_class( $class ) {
		return str_replace( "class='comment-reply-link", "class='comment-reply-link btn btn-outline-secondary", $class );
	}
	add_filter( 'comment_reply_link', 'sog_master_replace_reply_link_class' );
	 ...
	 */
	function sog_master_comment( $comment, $args, $depth ) {
	 ...
	 * @return array
	 */
	function sog_master_custom_commentform( $args = array(), $post_id = null ) {
		if ( null === $post_id ) {
			$post_id = get_the_ID();
		}
	...
	add_filter( 'comment_form_defaults', 'sog_master_custom_commentform' );
}
 ...
 * @return void
 */
function sog_master_scripts_loader() {
...
add_action( 'wp_enqueue_scripts', 'sog_master_scripts_loader' );
